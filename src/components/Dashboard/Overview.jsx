  ...
  const searchLocaton = async (e) => {
    ...
    try {
      const response = await axios.get(
        `https://api.geoapify.com/v1/geocode/search?text=${searchData}=&apiKey=815ef87723864d50a2ab9d3c87fd01af`
      );
    ...
    <div className="md:pl-[125px] px-5 md:pr-[100px] pt-[50px] pb-[15px] ">
      ...
      {Object.keys(data).length > 0 ? (
        <div className="pt-5 flex gap-5 w-full">
          <div className="flex flex-col gap-5 lg:w-[40%]">
            <div className="border rounded-[20px] p-6 bg-[#F5F5F5]">
              ...
              <p className=" text-sm font-medium text-[#000]">
                {data && data.indexes[0] && data.indexes[0].displayName}
              </p>
              <div className="flex flex-col items-center gap-[0px] pt-[14px]">
                <div>
                  <IgrRadialGauge
                    ...
                    width="350px"
                    minimumValue={0}
                    value={data && data.indexes[0] && data.indexes[0].aqi}
                    maximumValue={400}
                    interval={50}
                ...
                <div>
                  ...
                  <p className=" text-xs font-normal text-[rgba(0, 0, 0, 0.50)]">
                    ...
                    {data &&
                      data.indexes[0] &&
                      data.indexes[0].dominantPollutant}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="border rounded-[20px] p-6 bg-[#F5F5F5] lg:w-[35%]">
            {" "}
            <div className="">
              <h4 className=" text-2xl font-medium text-[#000] pb-1">
                Air Quality Index
              ...
              <div className="flex items-center gap-[14px] pt-[14px]">
                ...
                <div>
                  ...
                  <p className=" text-xs font-normal text-[rgba(0, 0, 0, 0.50)]">
                    Dominant pollutant:{" "}
                    {data && data.indexes[0].dominantPollutant}
                  </p>
                </div>
              </div>
              <div className="flex flex-col gap-[14px]">
                {data &&
                  data.pollutants.map((item, index) => (
                    <div
                      key={index}
                      className="border rounded-[10px] bg-[#D9D9D9] flex justify-between items-center py-[13px] px-[10px] mt-3 "
                    >
                      <div>
                ...
                {data.healthRecommendations &&
                  Object.keys(data.healthRecommendations).map((key, index) => (
                    <div
                      key={index}
                      className="border rounded-[10px] bg-[#D9D9D9] flex justify-between items-center py-[13px] px-[10px] mt-3"
                    >
                      <div>
                        <h6 className="text-sm font-bold text-[#000]">
